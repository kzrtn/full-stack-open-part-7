    ...
    add: async (blogFields) => {
      try {
        ...
        useNotificationStore
          ...
          .setNotification(
            "error",
            `Failed to submit blog post. Error: ${error}`,
          );
      }
    },
    like: async (updatedBlog) => {
      try {
        ...
        useNotificationStore
          ...
          .setNotification(
            "error",
            `Failed to like blog post. Error: ${error}`,
          );
      }
    },
    remove: async (blogToDelete) => {
      try {
        ...
        useNotificationStore
          ...
          .setNotification(
            "error",
            `Failed to delete blog post. Error: ${error}`,
          );
      }
    },
  },
  ...
  actions: {
    ...
    login: async (userObj) => {
      try {
        ...
        useNotificationStore
          .getState()
          .setNotification("success", `${user.name} successfully logged in.`);
      } catch (error) {
        useNotificationStore
          .getState()
          .setNotification("error", `Invalid credentials. Error: ${error}`);
      }
    },
    logout: () => {
      set({ user: null });
